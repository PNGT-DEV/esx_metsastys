...
local Positions = {
	['StartHunting'] = { ['hint'] = '[E] Aloita Metsästys', ['x'] = -769.23773193359, ['y'] = 5595.6215820313, ['z'] = 33.48571395874 },
	['Sell'] = { ['hint'] = '[E] Myy Hirven Lihaa', ['x'] = 969.16375732422, ['y'] = -2107.9033203125, ['z'] = 31.475671768188 },
			...
			for index, value in pairs(Positions) do
				if value.hint ~= nil then
					...
					if OnGoingHuntSession and index == 'StartHunting' then
						value.hint = '[E] Lopeta Metsästys'
					elseif not OnGoingHuntSession and index == 'StartHunting' then
						value.hint = '[E] Aloita Metsästys'
					end
	...
	if OnGoingHuntSession then
		...
		Citizen.CreateThread(function()
			...
			for index, value in pairs(AnimalPositions) do
				...
				BeginTextCommandSetBlipName("STRING")
				AddTextComponentString('Hirvi Havainto')
					...
					if DoesEntityExist(value.id) then
						...
						if AnimalHealth <= 0 then
							...
							if PlyToAnimal < 2.0 then
								...
								ESX.Game.Utils.DrawText3D({x = AnimalCoords.x, y = AnimalCoords.y, z = AnimalCoords.z + 1}, '[E] Kerää Liha', 0.4)
								...
								if IsControlJustReleased(0, Keys['E']) then
									if GetSelectedPedWeapon(PlayerPedId()) == GetHashKey('WEAPON_KNIFE')  then
										...
										ESX.ShowNotification('Tarvitset veitsen kerätäksesi lihan!')
									end
								end
...
function SlaughterAnimal(AnimalId)
	...
	ESX.ShowNotification('Keräsit hirven lihan ja sait ' ..AnimalWeight.. 'kiloa lihaa')
